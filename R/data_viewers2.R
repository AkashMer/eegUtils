    ...
    output$time_plot <- renderPlotly({
      ...
      init_plot <- tmp_data() %>%
        ...
        subplot(nrows = length(unique(tmp_data()$electrode)),
                shareX = TRUE, margin = c(0.5,0,0,0)) %>%
        hide_legend()
...
# Test
# data_dir <- "C:/Users/Akash Mer/Documents/project/learning_eeg/data/n170"
# set_file <- paste0(data_dir, "/sub-001/eeg/sub-001_task-N170_eeg.set")
# low_cut <- 0.1
# hi_cut <- 30
...
# chan_file <- paste0(data_dir, "/sub-001/eeg/sub-001_task-N170_electrodes.tsv")
